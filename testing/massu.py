...
class MachineVisionGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.cam1_img.setScaledContents(True)  # Scale image to fill the label
        ...
        self.cam2_img.setScaledContents(True)  # Scale image to fill the label
        ...
        self.cam3_img.setScaledContents(True)  # Scale image to fill the label
        ...
        # Right Side Layout - Reduced width
        ...
        # Control Panel - Only keep Start, Stop, Export buttons
        ...
        # Only three buttons: Start, Stop, Export
        button_layout = QHBoxLayout()
        ...
        # Adjust main layout proportions - give more space to cameras
        main_layout.addWidget(camera_widget, 4)  # Increased from 3 to 4
        main_layout.addWidget(right_widget, 1)
        
        self.setup_connections()

    def setup_camera_threads(self):
        # Start 3 camera threads for indices 0, 1, 2
        for idx in range(3):
    ...
    def update_camera_view(self, cam_index, frame):
        # Resize frame to fit the camera view while maintaining aspect ratio
        h, w = frame.shape[:2]
        
        # Get the size of the camera label
        if cam_index == 0:
            label_size = self.cam1_img.size()
        elif cam_index == 1:
            label_size = self.cam2_img.size()
        elif cam_index == 2:
            label_size = self.cam3_img.size()
        else:
            return
            
        # Calculate scaling to fit the frame in the label
        ...
        if cam_index == 0:
            self.cam1_img.setPixmap(pixmap)
            self.captured_frames[0] = frame
        elif cam_index == 1:
            self.cam2_img.setPixmap(pixmap)
            self.captured_frames[1] = frame
        elif cam_index == 2:
            self.cam3_img.setPixmap(pixmap)
            self.captured_frames = frame
    ...
    def update_camera_sizes(self):
        if hasattr(self, 'camera_frames'):
            available_width = self.width() - 350  # Adjusted for new right panel width
    ...
    def setup_connections(self):
        # Only connect the three remaining buttons
